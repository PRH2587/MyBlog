...
function launchParticlesJS(tag_id, params) {
	...
	if (pJS.retina_detect && window.devicePixelRatio > 1) {
		...
		pJS.particles.anim.speed = pJS.particles.anim.speed
				* pJS.canvas.pxratio;
		pJS.particles.line_linked.distance = pJS.particles.line_linked.distance
				* pJS.canvas.pxratio;
		pJS.particles.line_linked.width = pJS.particles.line_linked.width
				* pJS.canvas.pxratio;
		pJS.interactivity.mouse.distance = pJS.interactivity.mouse.distance
				* pJS.canvas.pxratio;
	}
	...
	pJS.fn.particle = function(color, opacity, position) {
		...
		/* size */
		this.radius = (pJS.particles.size_random ? Math.random() : 1)
				* pJS.particles.size;
		if (pJS.retina)
		...
		/* draw function */
		this.draw = function() {
			pJS.canvas.ctx.fillStyle = 'rgba(' + this.color.r + ','
					+ this.color.g + ',' + this.color.b + ',' + this.opacity
					+ ')';
			pJS.canvas.ctx.beginPath();

			switch (pJS.particles.shape) {
			case 'circle':
				pJS.canvas.ctx.arc(this.x, this.y, this.radius, 0, Math.PI * 2,
						false);
				break;

			case 'edge':
				pJS.canvas.ctx.rect(this.x, this.y, this.radius * 2,
						this.radius * 2);
				break;

			case 'triangle':
				pJS.canvas.ctx.moveTo(this.x, this.y - this.radius);
				pJS.canvas.ctx.lineTo(this.x + this.radius, this.y
						+ this.radius);
				pJS.canvas.ctx.lineTo(this.x - this.radius, this.y
						+ this.radius);
	...
	pJS.fn.particlesDraw = function() {
		...
		for (var i = 0; i < pJS.particles.array.length; i++) {
			var p = pJS.particles.array[i];
			p.draw('rgba(' + p.color.r + ',' + p.color.g + ',' + p.color.b
					+ ',' + p.opacity + ')');
		}
	...
	pJS.fn.vendors.distanceParticles = function(p1, p2) {
		...
		var dx = p1.x - p2.x, dy = p1.y - p2.y, dist = Math.sqrt(dx * dx + dy
				* dy);
		...
		if (dist <= pJS.particles.line_linked.distance) {
			...
			pJS.canvas.ctx.strokeStyle = 'rgba('
					...
					+ color_line.b
					+ ','
					+ (pJS.particles.line_linked.opacity - dist
			...
			if (pJS.particles.line_linked.condensed_mode.enable) {
				var dx = p1.x - p2.x, dy = p1.y - p2.y, ax = dx
						/ (pJS.particles.line_linked.condensed_mode.rotateX * 1000), ay = dy
						/ (pJS.particles.line_linked.condensed_mode.rotateY * 1000);
				p2.vx += ax;
				p2.vy += ay;
			}
	...
	pJS.fn.vendors.interactivity.listeners = function() {
		...
		detect_el.onmousemove = function(e) {
			...
			if (detect_el == window) {
				...
				var pos_x = e.offsetX || e.clientX, pos_y = e.offsetY
						|| e.clientY;
			}
	...
	pJS.fn.vendors.interactivity.grabParticles = function(p1, p2) {
		...
		var dx_mouse = p1.x - pJS.interactivity.mouse.pos_x, dy_mouse = p1.y
				- pJS.interactivity.mouse.pos_y, dist_mouse = Math
				.sqrt(dx_mouse * dx_mouse + dy_mouse * dy_mouse);

		/*
		 * Check distance between 2 particles + Check distance between 1
		 * particle and mouse position
		 */
		if (dist <= pJS.particles.line_linked.distance
			...
			pJS.canvas.ctx.strokeStyle = 'rgba('
					...
					+ color_line.b
					+ ','
					+ (pJS.interactivity.line_linked.opacity - dist_mouse
							/ pJS.interactivity.mouse.distance) + ')';
			pJS.canvas.ctx.moveTo(p1.x, p1.y);
			pJS.canvas.ctx.lineTo(pJS.interactivity.mouse.pos_x,
